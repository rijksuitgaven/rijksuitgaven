-- Migration 048: Fix retention date filter
-- The previous filter (first_week >= since_date) excluded historical cohorts,
-- making retention useless for short date ranges (e.g. 7 days showed only current week).
-- Now shows all cohorts regardless of date range — with few users this is fine.
