...
import { ChevronRight, ChevronLeft, ChevronDown, ChevronUp, ChevronsUpDown, Download, FileSpreadsheet, Info, Search, MousePointerClick, AlertTriangle, SlidersHorizontal, Columns3 } from 'lucide-react'
      ...
      // Expand button column
      {
        ...
        cell: ({ row }) => (
          <button
            ...
            className="p-1 cursor-pointer hover:bg-[var(--gray-light)] rounded transition-colors group"
            aria-expanded={row.getIsExpanded()}
            aria-label={row.getIsExpanded() ? 'Rij inklappen' : 'Rij uitklappen'}
          >
            {row.getIsExpanded() ? (
              <ChevronDown className="h-4 w-4 text-[var(--navy-medium)] group-hover:text-[var(--pink)]" aria-hidden="true" />
            ) : (
              <ChevronRight className="h-4 w-4 text-[var(--navy-medium)] group-hover:text-[var(--pink)]" aria-hidden="true" />
            )}
          </button>
        ),
        size: 32,
