...
Extended entity resolution and implemented Dutch language-aware search.
...
3. **V1.1 Search**: Dutch word rules to filter false cognates (-ie/-iek pattern)
4. **Semantic filtering**: "politie" no longer matches "Politieke beweging DENK"
5. **V2.0 Planning**: Documented Cohere embed-multilingual-v3 for embeddings (~€1/month)

**Deployed & tested**: Entity resolution and Dutch search rules working ✅
...
| **Search relevance ranking** | Exact matches first, then starts-with, word boundary, contains |
...
**V1.1 Implementation:**
- Added `build_search_condition()` helper function to `modules.py`
- For searches ending in "-ie", uses regex: `term([^k]|$|\s)` to exclude "-iek" false cognates
- Applied to all 6 search locations: aggregated views, source tables, integraal, autocomplete
- Example: "politie" → Matches Politie, Politieacademie, Nationale Politie; Excludes Politieke beweging DENK

**Embeddings Overview (V2.0):**
- Convert text to vectors capturing meaning
