...
V0.9 beta launched with 10 testers. Set up staging environment (Railway frontend-only, shared backend/DB/Typesense, ~€5-10/month). Defined 3 deployment workflows with expert team. Built UX-039 Vergelijk (row pinning) on staging — TanStack Table native RowPinning API, pin up to 4 rows, export selection. 4 sessions, 8 commits (6 staging-only, 2 production).
...
| Hash | Message |
|------|---------|
| `c5207da` | Add staging URL to ALLOWED_HOSTS for auth routes |
| `8964c49` | Docs: staging environment setup |
| `88e3895` | Staging processes: 3 workflows, sync rules, CLAUDE.md constraints |
| `bea1241` | UX-039: Vergelijk — row pinning for side-by-side comparison |
| `19dca68` | UX-039: Fix tooltip readability — native title |
| `7e72d19` | UX-039: Move Wis selectie to toolbar |
| `495a869` | UX-039: Style 'Wis selectie' as bordered toolbar button |
| `d6e9c14` | Docs: Session 4 — UX-039 styling polish documented |

---

## Next Steps

1. **Test UX-039 on staging** — verify pinning, export selection, "Wis selectie" styling with beta-like data
2. **Monitor beta feedback** — check /team/statistieken and /team/feedback daily
3. **Batch promote to production** — when UX-039 and other staging features are ready
4. **UX-038 "Ook in" brainstorm** — improve "Gevonden in" column when search matches primary field
5. **Rate limiting** — Cloudflare free tier before public launch

---

**Session Status:** Complete (4 sessions, 8 commits)
