...
| `scripts/sql/020-normalize-recipient-indexes.sql` | Functional indexes for normalize_recipient() |
| `backend/app/services/modules.py` | normalize_recipient() in details, smart autocomplete match_type, simplified regex fallback, totals query |
| `backend/app/api/v1/modules.py` | Added match_type to Pydantic model and response, totals in meta |
| `app/src/types/api.ts` | Added TotalsData interface, updated ApiMeta and ModuleDataResponse |
| `app/src/lib/api.ts` | Pass through totals from API response |
...
| `6cdc47a` | UX: Instant CSS tooltips for truncated text (no delay) |
| `8c050cc` | Fix: Move data-tooltip to parent wrappers to avoid overflow clipping |
| `5602656` | Add Totals row to data table for search/filter results |
| `ee055a4` | Perf: Parallel query execution for search (750ms → ~250ms) |
| `322d05b` | Docs: Session 13 - Search performance parallel queries |
| `82b7cb3` | Docs: Update performance results and next priorities |
| `26fa779` | Perf: Move relevance sorting to Python (reverted) |
| `79dc334` | Revert: Query tuning changes (no performance improvement) |

---

### Session 12: Totals Row for Search/Filter Results

**Issue:** User requested a "Total" row at the bottom of the data table showing aggregated sums per year column and grand total when searching/filtering.

**Design Decision:** User chose Option B - "it always needs to be perfect on the numbers" - meaning accurate totals across ALL matching results, not just the visible page. This requires backend aggregation.

**Implementation:**

1. **Backend Service** (`backend/app/services/modules.py`):
   - Added totals query that SUMs all year columns and grand total
   - Both fast path (`_get_from_aggregated_view`) and slow path (`_get_from_source_table`) now return totals
   - Updated function signatures to return `tuple[list[dict], int, dict | None]`
   - Totals only calculated when searching/filtering (not on default view)

2. **API Response** (`backend/app/api/v1/modules.py`):
   - Added `totals` to meta response object when available
   - Format: `{"years": {2016: 0, 2017: 1000, ...}, "totaal": 12345678}`

3. **Frontend Types** (`app/src/types/api.ts`):
   - Added `TotalsData` interface
   - Updated `ApiMeta` and `ModuleDataResponse` to include `totals?: TotalsData | null`

4. **API Client** (`app/src/lib/api.ts`):
   - Updated `fetchModuleData` to pass through `totals` from API response

5. **Module Page** (`app/src/components/module-page/module-page.tsx`):
   - Added `totals={data?.totals}` prop to DataTable component

6. **Data Table** (`app/src/components/data-table/data-table.tsx`):
   - Added `TotalsData` interface and `totals` prop
   - Implemented `<tfoot>` with totals row:
     - Navy dark background matching header
     - "Totaal" label with recipient count
     - Empty cells for extra columns
     - Collapsed years sum (2016-2020) when applicable
     - Individual year totals
     - Grand total with navy medium background

**Result:** When searching or filtering, a styled totals row appears at the bottom showing accurate sums across ALL results.

**Files Modified:**
| File | Changes |
|------|---------|
| `backend/app/services/modules.py` | Added totals query to both fast and slow paths |
| `backend/app/api/v1/modules.py` | Include totals in meta response |
| `app/src/types/api.ts` | Added TotalsData interface, updated ApiMeta and ModuleDataResponse |
| `app/src/lib/api.ts` | Pass through totals from API response |
| `app/src/components/module-page/module-page.tsx` | Pass totals prop to DataTable |
| `app/src/components/data-table/data-table.tsx` | Render totals row in tfoot |
...
### Session 13: Search Performance Optimization (Parallel Queries)

**Issue:** Search performance was slow (~750ms) due to sequential query execution: main query → count query → totals query.

**Solution:** Use `asyncio.gather()` to execute all three queries in parallel.

**Implementation:**

1. **`_get_from_aggregated_view`** (lines 773-807):
   - Build list of coroutines for main query, count query, and optionally totals query
   - Execute all with `asyncio.gather(*coros)`
   - Extract results from array

2. **`_get_from_source_table`** (lines 1062-1091):
   - Same pattern applied to slow-path queries
   - Totals query included only when search/filter active

**Actual Results (tested after deployment):**
...
| instrumenten | "ministerie" (large) | ~144ms |

**Improvement:** ~750ms → ~130-280ms (60-80% faster)

**Files Modified:**
| File | Changes |
|------|---------|
| `backend/app/services/modules.py` | Parallel query execution in both `_get_from_aggregated_view` and `_get_from_source_table` |
...
### Session 14: Query Tuning Attempt (Reverted)

**Goal:** Further optimize search from ~200ms toward <100ms target.

**Approach Tried:**
- Move relevance sorting from PostgreSQL (regex CASE) to Python
- Eliminate expensive `~*` regex pattern matching in SQL

**Result:** No improvement
- Regex CASE was already fast on filtered result sets (Typesense pre-filters)
- Integraal search became very slow (13s) - different code path without Typesense
- Reverted changes in commit `79dc334`

**Learning:**
- The bottleneck isn't the relevance CASE - it's already efficient on small sets
- Real optimization path: Typesense data enrichment (skip PostgreSQL entirely)
- Added to V1.1 backlog

**Decision:** Accept ~130-280ms for V1.0 launch. Optimize in V1.1.

**Files Modified:**
- Reverted `backend/app/services/modules.py` to previous state

**Commits:**
- `26fa779` - (reverted) Perf: Move relevance sorting to Python
- `79dc334` - Revert: Query tuning changes (no performance improvement)
