...
6. **UX-025: Feedback Button with Screenshot Selection** — Brainstormed beta feedback mechanisms with expert team (research lead, frontend engineer, customer success, adversarial strategist). Evaluated 4 options: built-in form, third-party widget, manual (WhatsApp/email), hybrid. Decision: email via Resend with area-selected screenshot (must-have). Wrote full requirement as UX-025 (V1.0, P1). Upgrade path: swap to GitHub Issues post-beta.

7. **Analytics & user tracking GDPR brainstorm** — Assembled expert team (privacy engineer, analytics lead, frontend performance, adversarial strategist). Full GDPR analysis of tracking options: server logs (no consent), Plausible/Umami (no consent for basic pageviews), PostHog/GA4 (consent required). Phased approach: logs now → Plausible post-launch → server-side events at 200+ users → PostHog at 500+. Decision: parked for V1.1/V1.2. Updated backlog with full analysis.

8. **CLAUDE.md UX tracker** — Updated next available UX number to UX-026.
...
| `scripts/typesense/sync_to_typesense.py` | Updated SQL + doc mapping for year amounts from universal_search |
| `backend/app/services/modules.py` | Added `_typesense_search_recipient_keys()` + integraal hybrid search |
| `02-requirements/search-requirements.md` | Added UX-024 type-ahead requirement + UX-025 feedback button requirement |
| `docs/VERSIONING.md` | Updated V1.1 entry with UX-024, added UX-025 to V1.0 features |
| `CLAUDE.md` | UX tracker → UX-026 |
