...
### Session 8: Browser & Device Analytics (2 commits)
1. **User-agent parsing** — `parseUserAgent()` in BFF events route. Detects browser (Chrome/Firefox/Safari/Edge/Opera/Onbekend) and device (Desktop/Mobiel/Tablet) from request header. Injected into every event's `properties` JSONB — no schema migration, uses existing GIN index.
2. **SQL function** — `get_usage_devices(since_date)` aggregates browser/device from properties JSONB, split by scope (public vs logged_in). Migration 064.
3. **BFF route** — one additional RPC call (`get_usage_devices`) added to parallel query batch in `/api/v1/team/statistieken`.
4. **Dashboard UI** — `DevicesSection` component: browser horizontal bars with percentages, device types with icons (Monitor/Smartphone/Phone). Shown on both Gebruik tab (logged-in events) and Website tab (public events).
...
| `scripts/sql/064-usage-devices.sql` | Browser & device aggregation function |
...
| `app/src/app/api/v1/events/route.ts` | Added `parseUserAgent()` — injects browser + device into every event's properties |
| `app/src/app/api/v1/team/statistieken/route.ts` | Added `get_usage_devices` RPC call to parallel query batch |
| `app/src/app/team/statistieken/page.tsx` | Added `DevicesSection` component on both Gebruik and Website tabs |
...
| `064-usage-devices.sql` | `get_usage_devices()` function — browser/device aggregation from properties JSONB | Yes |
...
| Browser/device in JSONB properties (not columns) | No migration needed, GIN index covers queries, data starts accumulating immediately |
| Server-side UA parsing (not client-side) | Reliable — header always present; no client changes; can't be spoofed by JS |
...
**Commits:** 14 (`0dd5e5f`, `634ac74`, `b6e6c0e`, `fc52cb7`, `baf6c51`, `3533b36`, `61079d5`, `1f8cf96`, `4c52862`, `178d83e`, `a5b4f44`, `954a7c6`, `e9a50dc`, `39c7d47`)
