...
- Root cause: `useEffect` reads localStorage one frame after render — component mounts with `unseen = []`, effect runs, brief flash
- Fix: `useState(getUnseenNotes)` — synchronous localStorage read during initialization, no useEffect needed
- SSR-safe: `typeof window === 'undefined'` guard returns `[]` on server
- Commit (main): `f07467e` — deployed to both environments
...
- Deeper root cause: 7 files tracking overlapping status → Consistency Tax guarantees divergence
...
- V2.0 status: 🔨 ~95% → ✅ Live (all 17 features confirmed with user)
- V2.0 features: bullet list → table format with per-feature ✅ Live (matches A1.0 pattern)
- Added V2.0.1 (Feb 21) + V2.0.2 (Feb 24) patch history
- V2.1 renamed: "Vergelijk" → "Vergelijk & URL State" (UX-041 moved from V2.2)
- V2.2 renamed: "Zoeken & URL State" → "Zoeken"
- Applied standardized status vocabulary: 📋/🔨/🧪/✅/⏳
- Regenerated roadmap-data.ts
- Commit: `e082d10` — deployed to both environments
...
- Commit: `7f987cc` — deployed to both environments

### 23. Backlog Cleanup + Source-of-Truth Pointers
- `backlog.md`: 1406 → 350 lines (75% reduction)
...
- Regenerated roadmap-data.ts
- Commit: `32602aa` — deployed to both environments

## Files Modified (additional — Session 8)

| File | Changes |
|------|---------|
| `docs/VERSIONING.md` | V2.0 ✅ Live, table format, patches, V2.1 renamed, status vocabulary |
| `02-requirements/backlog.md` | Complete rewrite: 1406→350 lines, future-only, organized by track |
...
## Issues Resolved (additional — Session 8)
...
| UX-041 shown in V2.2 (actually V2.1 staging) | Moved to V2.1, status updated to 🧪 On staging |
...
- Commit: `c363c90` — deployed to both environments
...
- Regenerated `roadmap-data.ts`
- Commit: `bd0d54c` — deployed to both environments

### 26. V5.0 Inzichten Dashboard — 6 Visualization Prototypes (Lab h6)
...
- Built 6 BFF routes under `/api/v1/inzichten/`:
  1. **Spending Pulse** (`/pulse`) — KPI cards + stacked area chart by module
  2. **New Money** (`/new-entrants`) — First-time recipient detector with year selector + threshold
  3. **The Movers** (`/movers`) — Top-N ranked line chart with pin/compare (3 pinned)
  4. **Ministry DNA** (`/ministry-dna`) — Sparkline matrix (5 metrics × 9 years per ministry)
  5. **Spending Landscape** (`/spending-tree`) — D3 zoomable treemap (Ministry → Regeling → Instrument)
  6. **Dependency Radar** (`/cross-module`) — D3 radial layout showing cross-module presence
- Fixed 5 TypeScript strict mode errors (Supabase dynamic column access + Recharts formatter signatures)
- Commit: `d3da6dd` — deployed to both environments

## Files Created (additional — Session 9)
...
## Files Modified (additional — Session 9)
...
## Issues Resolved (additional — Session 9)
...
### Also completed (separate session)
- ✅ Onboarding email sequence (5 emails implemented)
