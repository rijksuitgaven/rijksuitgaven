...
### Session 9: UX-036 Public Page Analytics (multi-session, cross-context)
1. **2 new event types** — `public_page_view` (page loads) + `public_interaction` (user actions). Added to `use-analytics.ts` hook (`publicSessionId` via `crypto.randomUUID()`) and `events/route.ts` VALID_EVENT_TYPES whitelist.
2. **TrackPageView shared component** — `components/analytics/track-page-view.tsx`, client component for server-rendered pages. Fires `public_page_view` with session_id, referrer hostname, UTM params on mount.
3. **8 public pages tracked** — privacybeleid, voorwaarden, verlopen, afmelden, login, over, support, dataoverzicht: each got `<TrackPageView page="..." />`.
4. **Login tracking** — `login-form.tsx`: `login_attempt` + `login_magic_link_sent` events with `publicSessionId`.
5. **Homepage tracking (CORRECTED)** — Initially added to wrong file (`public-homepage.tsx` = h7 prototype). Discovered via user screenshot that live homepage is `homepage.tsx`. Fixed: page_view on mount, 7 section_views via ScrollReveal `onVisible`, 8 CTA clicks (bekijk_alle_data, 3× neem_contact, discovery ontdek_meer, 3× share buttons), contact form start/submit.
6. **SQL migration 062** — 8 RPC functions for public analytics (page_views, interactions, contact_funnel, referrers, cta_clicks, scroll_funnel, login_funnel, utm_campaigns). Executed on production.
7. **Dashboard Website tab** — Gebruik/Website tab bar on `/team/statistieken`. WebsiteSection component with KPI cards, page views breakdown, scroll funnel, CTA clicks table, referrers, UTM campaigns.
8. **Dashboard API** — 8 new RPC queries in `statistieken/route.ts` Promise.all, response under `public` key.
9. **Cleanup** — Deleted `/h7` page (prototype, not live). Removed "Bekijk alle jaren en data" CTA from homepage. Added clarifying comments to both homepage files. Updated MEMORY.md with homepage component mapping.

## Files Created (Session 9)

| File | Description |
|------|-------------|
| `app/src/components/analytics/track-page-view.tsx` | Shared page view tracking component |
| `scripts/sql/062-public-analytics.sql` | 8 RPC functions for public analytics |

## Files Modified (Session 9)

| File | Changes |
|------|---------|
| `app/src/hooks/use-analytics.ts` | 2 new event types, publicSessionId |
| `app/src/app/api/v1/events/route.ts` | 2 new VALID_EVENT_TYPES |
| `app/src/components/homepage/homepage.tsx` | Full tracking: page_view, 7 section_views, 8 CTA clicks, contact form. Removed "Bekijk alle jaren en data" CTA. Added file header comment. |
| `app/src/components/homepage/public-homepage.tsx` | Tracking added (h7 only). Added file header comment. |
| `app/src/components/auth/login-form.tsx` | login_attempt + login_magic_link_sent tracking |
| `app/src/app/privacybeleid/page.tsx` | TrackPageView added |
| `app/src/app/voorwaarden/page.tsx` | TrackPageView added |
| `app/src/app/verlopen/page.tsx` | TrackPageView added |
| `app/src/app/afmelden/page.tsx` | TrackPageView added |
| `app/src/app/login/page.tsx` | TrackPageView added |
| `app/src/app/over/page.tsx` | TrackPageView added |
| `app/src/app/support/page.tsx` | TrackPageView added |
| `app/src/app/dataoverzicht/page.tsx` | TrackPageView added |
| `app/src/app/api/v1/team/statistieken/route.ts` | 8 public analytics RPC queries |
| `app/src/app/team/statistieken/page.tsx` | Website tab + WebsiteSection component |
| `02-requirements/search-requirements.md` | UX-036 marked implemented |
| `CLAUDE.md` | UX counter updated |

## Files Deleted (Session 9)

| File | Reason |
|------|--------|
| `app/src/app/h7/page.tsx` | Prototype page, not live site |

## SQL Migrations (Session 9)

| Migration | Description | Executed |
|-----------|-------------|----------|
| `062-public-analytics.sql` | 8 public analytics RPC functions | Yes |

## Issues Resolved (Session 9)

| Issue | Solution |
|-------|----------|
| Tracking added to wrong homepage component | Discovered `public-homepage.tsx` is h7 prototype. Live homepage is `homepage.tsx`. Fixed tracking. Added clarifying comments. Updated MEMORY.md. |
...
- Deploy UX-036 changes to production

---

**Commits:** 23 (6 session 1 + 2 docs + 1 session 2 + 1 session 3 + 1 docs + 1 session 4 + 1 docs + 1 session 5 + 1 docs + 3 session 6 + 2 docs + 1 session 7 + 1 session 8 + 1 session 9)
