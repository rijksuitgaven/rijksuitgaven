...
| `02-requirements/search-requirements.md` | Added UX-035 Datasets Page requirement |
| `scripts/data/DATA-UPDATE-RUNBOOK.md` | Added per-entity year_to guidance + verification queries |
| `app/src/app/api/v1/datasets/route.ts` | BFF route for data availability (auth-gated Supabase query) |
| `app/src/app/datasets/page.tsx` | Datasets page with dynamic availability matrices |
...
### 16. UX-035 Datasets Page — Dynamic Data Availability Matrix
- Built `/datasets` page (post-login) with dynamic availability from `data_availability` table
- BFF route `GET /api/v1/datasets` — auth-gated, queries Supabase
...
- Added "Datasets" link to footer (logged-in state only)
- UX-035 status updated to ✅ Implemented 2026-02-18

### 17. Datasets Page — Remove Source Links
