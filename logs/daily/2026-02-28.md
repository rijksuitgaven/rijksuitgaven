...
Massive bug-fix day: 7 production fixes + 10 staging UX-039 pin/expand fixes across 2 sessions. Session 1 focused on production: word boundary matching, NULLS LAST sort, browser back button, version renumbering, test framework (32/32 pass), login email copy. Session 2 was a deep TanStack Table debugging saga for row pinning — 6 root causes found (parseInt on string IDs, stale useMemo closure, sticky column overlap, server-side pagination cache gap, data-change state wipe, hover on colored bg). Added Virtual Senior Team protocol to CLAUDE.md. 20 commits total (7 main, 11 staging, 2 docs).
...
1. **UX-039 final testing** — Full pin/unpin/expand/export flow on staging before production approval
...
| CLAUDE.md staging registry updated | ✅ New code markers added |
| Daily log complete | ✅ |
