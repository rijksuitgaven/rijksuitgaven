...
- ✅ **2026-02-15 (Session 12):** Autocomplete dropdown fix — dropdown stayed open after Enter on highlighted result. Root cause: changing search value triggered autocomplete effect which reopened dropdown. Fixed by resetting `hasUserTypedRef` before setting value.
...
- ✅ **2026-02-20 (Session 1):** UX-037 Concept E-mails — draft save/edit/delete for /team/mail. SQL migration 063 (status column, nullable sent_at, updated_at). Drafts API (POST/PUT/DELETE). Send route accepts draftId for draft→sent conversion. Frontend: "Opslaan als concept" button, Concepten/Verzonden split in campaigns tab, segment badges, draft indicator bar. 1 commit.
- ✅ **2026-02-20 (Session 2):** Legal entity name update — Het Maven Collectief → Rijksuitgaven.nl (new handelsnaam, same KVK 96257008). 7 changes across voorwaarden, privacybeleid, email templates. 1 commit.
- ✅ **2026-02-20 (Session 3):** Database column naming audit — expert panel (4 specialists + adversarial). Confirmed all 6 naming inconsistencies safely abstracted by MODULE_CONFIG. No changes needed. 0 commits.
